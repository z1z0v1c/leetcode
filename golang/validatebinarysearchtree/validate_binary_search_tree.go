...
func isValidBST(root *cs.TreeNode) bool {
	return isValidSubtree(root, math.MinInt, math.MaxInt)
}

func isValidSubtree(node *cs.TreeNode, min, max int) bool {
	if node == nil {
		return true
	}
	...
	isValid := isValidSubtree(node.Left, min, node.Val)
	if !isValid {
		return false
	}
	
	return isValidSubtree(node.Right, node.Val, max)
}